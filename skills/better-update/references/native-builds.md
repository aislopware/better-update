...
| `--freeze-credentials`              | off          | Fail fast if credentials are missing instead of prompting (CI). A stale (roster-changed) profile still regenerates headless when the Apple team has an ASC key. |
