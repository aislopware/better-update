...
### OTA on a non-Expo project type

A `bare` / `native` / `kmp` / `custom` project can still ship `expo-updates` — committing `ios/` +
`android/` precisely so `expo prebuild` never overwrites them is a normal setup. The project type
decides **how the binary is produced**; it does not decide whether the project does OTA. That answer
comes from `expo-updates` being a dependency.

When it is, two things happen on every build, on every strategy:

- **The `channel` is baked into the binary** — merged into `expo-channel-name` inside
  `AndroidManifest.xml` / `Expo.plist`, whether those files were generated by prebuild or committed
  to the repo. A binary without it silently reads the server's **default** channel, so your update
  publishes fine and reaches nobody.
- **The `runtimeVersion` is read from the Expo config** and recorded on the build, which is what
  `builds compatibility-matrix` needs to tell you whether an update can reach the field.

> **A profile with `expo-updates` installed must declare a `channel`.** The build **fails** without
> one rather than producing a binary pinned to the default channel. This is a hard error, not a
> warning — the failure it replaces was invisible until you unpacked the APK.

Set it per profile in `eas.json`:
...
```json
{
  ...
  "build": {
    ...
    "staging": { "channel": "staging", "environment": "preview" }
  }
}
```
