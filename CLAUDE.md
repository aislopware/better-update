...
Turborepo + bun workspaces (`apps/*`, `packages/*`); releases via Lerna (conventional commits). Run any task from root — `bun run <task>` fans out via `turbo`.

- `apps/server` — Cloudflare Worker, Effect `HttpApi` (architecture below).
- `apps/web` — TanStack Start dashboard, SSR, deploys to Cloudflare.
- `apps/cli` — Effect CLI (EAS-like build/update tooling).
- `apps/docs` — Astro docs site.
- `packages/*` — shared libs: `api` (contracts), `api-client`, `auth-client`, `credentials-crypto`, `ui`, plus encoding/type-guards/safe-json/etc.

## Tooling

- Use `bun`/`bunx` for scripts + deps. No `npm`/`npx`/`yarn`.
- Use `bun run lint` for lint + typecheck. No `oxlint` or `tsgo`/`tsc` direct.
- Format with `oxfmt` (`bun run format`) — not prettier.
- Server dev runs behind a portless proxy (`bun run dev:proxy` / `dev:trust` to set up). Deploy = `bun run deploy` (`vite build && wrangler deploy`).
...
| Imperative shell | `durable-objects/`                                    | DO classes for publish coordination; bridge Effect programs into the DO runtime via `effect-interop`. `publish-types.ts` = pure contract shared with `application/` | `effect`, `cloudflare:workers`, `application/`, `cloudflare/`, `infrastructure-layer` | —                                                                                                          |
