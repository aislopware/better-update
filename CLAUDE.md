...
- `Effect.promise` allowed only in `repositories/` + `cloudflare/*Live` — I/O boundary. Elsewhere, compose Effect services, no wrap raw async.
- `domain/` + `http/` stay pure. Logic need I/O → repository or adapter, not here.
