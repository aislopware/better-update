...
type ToastOptions = Omit<Parameters<typeof toastManager.add>[0], "title" | "type">

// Sonner-shaped sugar over the Base UI manager: `type` is what <ToastList />
// reads to pick the icon, so raising a toast should not have to spell it out.
// Reach for `toastManager` directly for anything past these two.
const toast = {
...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        // z-100, not the registry's z-50: dialogs/popovers/sheets are also z-50
        // and portal in *after* the toast viewport, so an equal z-index would
        // put every toast raised from an open dialog behind its backdrop.
