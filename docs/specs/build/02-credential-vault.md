...
resolved **build-system-agnostically** — `BETTER_UPDATE_PROJECT_ID` env > `eas.json` (top-level `projectId`) >
...
| Mode                         | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Keychain cache (default)** | First decrypt in a session prompts for the passphrase, unlocks the identity, fetches + unwraps the vault key, and caches it in the OS keychain (`@napi-rs/keyring` — macOS Keychain / Windows Credential Manager / Linux libsecret) for **15 minutes** by default (`credentials unlock --duration` picks a longer or shorter window). Subsequent commands read the cached key and skip both the prompt and the Argon2id derivation until it expires. |
| **In-process memo**          | Within ONE command the opened identity is remembered in memory, so a run that unlocks more than one vault — `credentials access grant` (credentials + env), `credentials account create` (verify + env), a build reading credentials then env values — prompts once instead of once per vault. It never outlives the process and is written only after the passphrase actually opened the identity.                                                  |
| **Prompt**                   | `BETTER_UPDATE_NO_CACHE=1` (or no usable OS keychain — keyring access degrades to "no cache" rather than failing) prompts every time a decrypt is needed — but still at most once per command, via the in-process memo above.                                                                                                                                                                                                                        |
...
- `better-update credentials unlock [--duration <90|45m|2h|1h30m>]` — prompt once, unwrap, and cache
  the vault key for the chosen TTL (default 15 minutes, bounded to 1 minute – 24 hours).
