...
// project id from BETTER_UPDATE_PROJECT_ID / eas.json — never Expo.
...
describe("generic vault: credentials/env on a non-Expo project", () => {
  ...
  it("env list without any link source fails with the not-linked guard", () => {
    ...
    // No env var, no eas.json yet, no Expo config → ProjectNotLinkedError.
  ...
  it("init --name/--slug links a non-Expo project by writing eas.json", () => {
  ...
  it("doctor reports the project as linked via eas.json", () => {
