...
describe(regenerateProvisioningProfile, () => {
  ...
  it.effect("regenerates headless off the team's ASC key in non-interactive mode", () =>
    Effect.gen(function* () {
      const { api, updates } = buildApi(bundleConfig(null), [ascKey]);
  ...
  it.effect("fails in non-interactive mode when the team has no ASC key", () =>
    Effect.gen(function* () {
      const { api } = buildApi(bundleConfig(null), [
        { ...ascKey, id: "other", appleTeamId: "team-uuid-OTHER" },
      ]);

      const error = yield* Effect.flip(regenerateProvisioningProfile(api, input));

      expect(error._tag).toBe("MissingCredentialsError");
      expect(error.message).toContain("no App Store Connect API key is stored");
      expect(mocks.regenerateViaAppleId).not.toHaveBeenCalled();
    }).pipe(Effect.provide(stubLayer(false))),
  );

  it.effect("uses a caller-resolved ASC key without listing or prompting", () =>
    Effect.gen(function* () {
      const { api, updates } = buildApi(bundleConfig(null), []);

      const created = yield* regenerateProvisioningProfile(api, input, {
        ascApiKeyId: "asc-key-from-resolve",
      });
