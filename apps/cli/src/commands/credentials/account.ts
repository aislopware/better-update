...
const linkCommand = Command.make(
  ...
  Effect.fn(
    function* () {
      ...
      if (own === null) {
        return yield* new IdentityError({
          message: "No account key enrolled. Run `better-update credentials account create` first.",
        });
      }
      const cutOver = yield* orgHasCutOver(api);
      if (!cutOver) {
...
const resealCommand = Command.make(
  "reseal",
  {},
  Effect.fn(
    function* () {
...
const showCommand = Command.make("show", {}, showHandler).pipe(
  Command.withDescription("Show this user's enrolled account key (fingerprint + status)"),
);

export const accountCommand = Command.make("account", {}, showHandler).pipe(
  Command.withDescription("Manage your per-user account key for browser-side env-vault access"),
  Command.withSubcommands([createCommand, linkCommand, resealCommand, showCommand]),
);
