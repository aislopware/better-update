  ...
  return (
    ...
    >
      <div className="bg-kumo-tint relative h-7 overflow-hidden rounded-lg" aria-hidden="true">
        {oldPercent > 0 ? (
          <div
            className="bg-kumo-badge-neutral/70 absolute inset-y-0 left-0 transition-[width] duration-300"
            style={{ width: `${oldPercent}%` }}
          />
        ) : null}
        {newPercent > 0 ? (
          <div
            className="bg-kumo-contrast absolute inset-y-0 right-0 transition-[width] duration-300"
            style={{ width: `${newPercent}%` }}
          />
        ) : null}
        {oldPercent > 0 && newPercent > 0 ? (
          <div
            className="bg-kumo-base absolute inset-y-0 w-px transition-[left] duration-300"
            style={{ left: `${oldPercent}%` }}
          />
        ) : null}
      </div>
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="text-kumo-subtle flex items-center gap-1.5">
          <span className="bg-kumo-badge-neutral/70 size-2 rounded-full" aria-hidden="true" />
        ...
        <div className="text-kumo-default flex items-center gap-1.5">
          <span className="bg-kumo-contrast size-2 rounded-full" aria-hidden="true" />
          <GitBranchIcon weight="bold" className="size-3" />
          ...
          {newPercent === 100 ? (
            <CheckCircleIcon
              weight="bold"
              className="text-kumo-success size-3.5"
