...
const setProtectionEffect = (id: string, isProtected: boolean) =>
  toApiCrudEffect(
    Effect.gen(function* () {
      ...
      yield* logAudit({
        action: isProtected
          ? "apple.distribution-certificate.protect"
          : "apple.distribution-certificate.unprotect",
        resourceType: "appleCredential",
        resourceId: id,
        metadata: { serialNumber: existing.serialNumber },
      });
      return toApiAppleDistributionCertificate({ ...existing, isProtected });
    }),
  );
      ...
      .handle("list", () =>
        toApiCrudEffect(
          Effect.gen(function* () {
            ...
            return { items: visible.map(toApiAppleDistributionCertificate) };
          }),
        ),
      )
