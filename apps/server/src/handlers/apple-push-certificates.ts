...
import { ApplePushCertificateRepo } from "../repositories/apple-push-certificates";
      ...
      .handle("list", () =>
        toApiCrudEffect(
          Effect.gen(function* () {
            ...
            return { items: items.map(toApiApplePushCertificate) };
          }),
        ),
      )
      .handle("upload", ({ payload }) =>
        toApiWriteEffect(
          Effect.gen(function* () {
            ...
            const r2Key = `apple-push-certificates/${ctx.organizationId}/${crypto.randomUUID()}.p12.enc`;
            ...
            yield* logAudit({
              action: "apple.push-certificate.upload",
            ...
            return toApiApplePushCertificate({
              id: payload.id,
              organizationId: ctx.organizationId,
              appleTeamId: team.id,
              bundleIdentifier: payload.bundleIdentifier,
      ...
      .handle("delete", ({ path }) =>
        toApiCrudEffect(
          Effect.gen(function* () {
            ...
            yield* logAudit({
              action: "apple.push-certificate.delete",
              resourceType: "appleCredential",
              resourceId: path.id,
      ...
      .handle("download", ({ path }) =>
        toApiBadRequestReadEffect(
          Effect.gen(function* () {
            ...
            yield* logAudit({
              action: "apple.push-certificate.download",
              resourceType: "appleCredential",
              resourceId: path.id,
              metadata: { serialNumber: existing.serialNumber },
            });
