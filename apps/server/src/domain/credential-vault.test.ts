...
describe("credential-vault", () => {
  describe(resolveKeyring, () => {
    it("parses valid keyring JSON", () => {
      ...
      expect(keyring.secrets[1]!).toHaveLength(32);
    });

    it("selects highest version as currentVersion", () => {
    ...
    it("throws on empty keyring", () => {
      expect(() => Effect.runSync(resolveKeyring("{}"))).toThrow("Vault keyring is empty");
    });

    it("returns a tagged config error on invalid JSON", async () => {
