...
const insertAccountKey = async (id: string, userId: string) =>
  env.DB.prepare(
...
describe("touchLastUsed — D1 integration", () => {
  it("stamps last_used_at on a user encryption key", async () => {
    await expect(keyLastUsedAt("user_encryption_keys", "klu-device")).resolves.toBeNull();
    ...
    await expect(keyLastUsedAt("user_encryption_keys", "klu-device")).resolves.toBe(
      "2026-07-06T12:00:00.000Z",
    );
  });

  it("stamps last_used_at on an account key", async () => {
    await expect(keyLastUsedAt("account_keys", "klu-account")).resolves.toBeNull();
    await runWithLayerAndEnv(
      Effect.gen(function* () {
    ...
    await expect(keyLastUsedAt("account_keys", "klu-account")).resolves.toBe(
      "2026-07-06T12:00:00.000Z",
    );
  });
});
