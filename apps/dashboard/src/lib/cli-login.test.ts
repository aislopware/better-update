...
describe("cli-login helpers", () => {
  test("accepts localhost callback URLs", () => {
  ...
  test("rejects non-local or https callback URLs", () => {
  ...
  test("builds callback redirect with token in hash", () => {
  ...
  test("builds redirect target back to cli-login route", () => {
  ...
  test("formats deterministic CLI API key names", () => {
